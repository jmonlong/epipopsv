...
```{r}
cnv.all %>% filter(project=="epilepsy") %>% group_by(sample) %>% summarize(kb=sum((end-start+1)/1e3)) %>% ggplot(aes(x=reorder(sample, kb), y=kb)) + geom_bar(stat="identity") + theme_bw() + ylab("affected genome (Kbp)") + theme(axis.text.x=element_blank()) + xlab("sample")
```
