...
cnv.s = res.df %>% filter(prop<.5, sample %in% twins)
load("../data/cnvs-PopSV-twin-5kbp-FDR05.RData")
res.df$sig = res.df$qv
cnv.l = data.frame(method="PopSV", set="loose", res.df[, c("sample","chr","start","end","sig")])
cnv.l = rbind(cnv.l, subset(others.df, set=="loose")[, c("method","set","sample","chr","start","end", "sig")])
...
```{r}
conc.tw = cnv.s %>% group_by(sample, method) %>% summarize(nb.c=sum(conc), prop.c=mean(conc)) %>% mutate(method = factor(as.character(method), levels=methods.f))
...
ggplot(conc.tw,aes(x=method,y=nb.c)) + geom_boxplot(aes(fill=method)) + theme_bw() + xlab("")  + ylab("number of replicated calls per sample") + coord_flip() + guides(fill=FALSE) + scale_fill_manual(values=cbPalette)
```
...
ggplot(subset(meth.2d, method != method2), aes(x=method2, fill=method, y=prop.call)) + geom_bar(stat="identity",position="dodge", data=meth.2d %>% filter(method!=method2) %>% group_by(method, method2) %>% summarize(prop.call=median(prop.call))) + geom_boxplot(alpha=0,position=position_dodge(.9)) + theme_bw() + scale_fill_brewer(name="call from", palette="Set1") + xlab("found by") + ylab("proportion of calls") + ylim(0,1)
```
...
ggplot(subset(meth.2d.p, method != method2), aes(x=method2, fill=method, y=prop.call)) + geom_bar(stat="identity",position="dodge", data=meth.2d.p %>% filter(method!=method2) %>% group_by(method, method2) %>% summarize(prop.call=median(prop.call))) + geom_boxplot(alpha=0,position=position_dodge(.9)) + theme_bw() + scale_fill_brewer(name="call from", palette="Set1") + xlab("found by") + ylab("proportion of calls") + ylim(0,1) + ggtitle("Calls in 2 methods or more")
```
...
```{r, echo=FALSE}
save(res.ave.samp, ri.df, ri.s, conc.tw, conc.tw.sig, meth.2d, meth.2d.p, file='PopSV-methodBenchmark-twin.RData')
```
