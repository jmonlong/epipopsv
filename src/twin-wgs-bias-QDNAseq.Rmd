---
title: Twin - Bias in Whole-Genome Sequencing Coverage - QDNAseq correction
author: Jean Monlong
output:
  md_document:
...
Using the [QDNAseq pipeline](https://www.ncbi.nlm.nih.gov/pubmed/25236618) ([Bioconductor package](https://www.bioconductor.org/packages/release/bioc/html/QDNAseq.html)), read depth was computed from the BAM files and corrected for GC and mappability bias. 
The script describing how QDNAseq was run on our cluster is in the `others` folder (file `QDNAseq-twins.R`).
...
bc.df = read.table("../data/bc-corrected-QDNAseq-twins.tsv.gz", as.is=TRUE, sep="\t", header=TRUE)
...
```{r, echo=FALSE}
save(bc.norm, sub.samples, bc.norm.sim, bc.norm.shuf, var.is, rk.m, file='twin-wgs-bias-QDNAseq.RData')
```
