...
```{r}
exon.cnv.genes = cnv.all %>% filter(project=="epilepsy") %>% makeGRangesFromDataFrame(keep.extra.columns=TRUE) %>% subsetByOverlaps(exons.grl$exon, .) %>% mcols %>% .$geneName %>% unique
...
```{r}
test.epi.genes = list(
  allGenesDelNoDB=testGenesCnvs(cnv.all %>% filter(project=="epilepsy", z<0, prop.db==0), nb.perm=1e3, nb.class=1),
  sizeGenesDelNoDB=testGenesCnvs(cnv.all %>% filter(project=="epilepsy", z<0, prop.db==0), nb.perm=1e3, nb.class=5)
)
